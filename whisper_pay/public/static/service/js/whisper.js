...
$(function () {
    ...
    $("#scroll-link").click(function(){
        ...
        $.getJSON('/service/index/reLink', function (res) {
            var _tab_html = '';
            var _user_html = '';
    ...
    $("#join-black").click(function () {
        ...
        layui.use(['layer'], function () {
            ...
            var index = layer.confirm('确定将其加入黑名单？', {
                btn: ['确定', '取消']
            }, function(){

                $.post('/service/index/joinBlack', {
                    customer_name: $("#active-user").attr('data-name'),
                    customer_ip: $("#active-user").attr('data-ip')
                }, function (res) {
                    if (0 == res.code) {
                        socket.send(JSON.stringify({
                            type: 'closeUser', uid: $("#active-user").attr('data-id')
                        }));
                        layer.close(index);
                    }
                });
...
function sendMessage(sendMsg) {
    ...
    if('http' == customerProtocolTable[uid]) {
        ...
        $.post('/service/index/sendMsgFromHttp', sendMsg, function (res) {
        ...
        if(1 == res.code && res.data.length > 0){
            ...
            if(res.msg == res.total){
                ...
                var _html = '<div class="layui-flow-more"><a href="javascript:;" data-page="' + parseInt(res.msg + 1)
                    + '" data-uid="' + uid + '" onclick="getMore(this)"><cite>更多记录</cite></a></div>';
            }
...
var closeUser = function(){
    ...
    $('.close').click(function () {
        ...
        if($(this).parent().find('img').hasClass('off-line')) {
            $.getJSON('/service/index/offlineHide', {id: $(this).parent().data('log')}, function (res) {});
        }else {
